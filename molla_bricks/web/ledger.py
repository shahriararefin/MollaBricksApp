...
def ledger_book():
    ...
    if party_name:
        where_clauses.append("party_name = ?")
    ...
    total_debit = sum(t[5] for t in transactions)
    ...
    return render_template('ledger_book.html', 
                           ...
                           balance=balance,
                           now=datetime.now()) # <-- THIS LINE IS THE FIX
...
def add_entry():
    ...
    try:
        ...
        flash('Credit and Debit must be valid numbers.', 'danger')
    ...
    if credit > 0 and debit > 0:
        flash('Please enter either a Credit or a Debit, not both.', 'warning')
        return redirect(url_for('ledger.ledger_book'))
...
def auto_settle_dues(customer_id, customer_name, payment_amount, ledger_entry_id):
    ...
    due_records = db_controller_instance.execute_query(dues_query, (customer_id,), fetch="all")
    ...
    for chalan_id, chalan_due in due_records:
        ...
        db_controller_instance.execute_query("UPDATE nagad_khata SET paid_amount = paid_amount + ?, due_amount = due_amount - ? WHERE id = ?", (payment_for_this_chalan, payment_for_this_chalan, chalan_id))
        
        desc = f"Advance applied to Chalan ID: {chalan_id} [AUTO_SETTLED_ID:{chalan_id}]"
        db_controller_instance.execute_query("INSERT INTO ledger_book (date, party_name, description, debit, credit) VALUES (?, ?, ?, ?, ?)", (datetime.now().strftime('%Y-%m-%d'), customer_name, desc, payment_for_this_chalan, 0))
    ...
    if settled_chalans:
        ...
        db_controller_instance.execute_query("UPDATE ledger_book SET description = description || ? WHERE id = ?", (settlement_desc, ledger_entry_id))