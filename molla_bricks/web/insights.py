...
@bp.route('/')
def index():
    """Renders the Insights page with customer analysis."""
    
    # Query for Most Valuable Customers (by total sales)
    valuable_query = """
    ...
    # Query for Most Frequent Customers (by number of sales)
    frequent_query = """
        SELECT customer_name, COUNT(id) as visit_count
        FROM nagad_khata
        WHERE customer_id IS NOT NULL
        GROUP BY customer_id, customer_name
