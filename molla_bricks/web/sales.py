...
def print_chalan(sale_id):
    query = "SELECT * FROM nagad_khata WHERE id = ?"; record = db_controller_instance.execute_query(query, (sale_id,), fetch="one")
    if not record: flash("Sale not found.", 'danger'); return redirect(url_for('sales.nagad_khata'))
    ...
    try:
        pdf_path = NagadService.generate_chalan_pdf(chalan_data); return send_file(pdf_path, as_attachment=True)
    except Exception as e:
        flash(f"Error generating PDF: {e}", 'danger'); return redirect(url_for('sales.nagad_khata'))
