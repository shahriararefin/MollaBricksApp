...
def add():
    try:
        ...
        if new_id:
            ...
            if due_amount > 0: auto_settle_from_advance(customer_id, customer_name)
...
def auto_settle_from_advance(customer_id, customer_name):
    ...
    if balance > 0:
        ...
        for chalan_id, chalan_due in due_records:
            ...
            db_controller_instance.execute_query("INSERT INTO ledger_book (date, party_name, description, debit, credit) VALUES (?, ?, ?, ?, ?)", (datetime.now().strftime('%Y-%m-%d'), customer_name, desc, payment_for_this_chalan, 0)); payment_to_apply -= payment_for_this_chalan